...
_rl_df = pd.DataFrame(_rl_data) if _rl_data else pd.DataFrame()
_has_logs = not _rl_df.empty
...
if _has_data:
    ...
    if not today_row.empty:
        ...
        if _has_logs:
            _today_logs = _rl_df[_rl_df["date"] == today]
            for _, _tl in _today_logs.iterrows():
                _tl_parts = []
                if pd.notna(_tl.get("distance_miles")):
                    _tl_parts.append(f"{float(_tl['distance_miles']):.2f} mi")
                if pd.notna(_tl.get("duration_seconds")) and int(_tl["duration_seconds"]) > 0:
                    _ds = int(_tl["duration_seconds"])
                    if _ds >= 3600:
                        _tl_parts.append(f"{_ds // 3600}:{(_ds % 3600) // 60:02d}:{_ds % 60:02d}")
                    else:
                        _tl_parts.append(f"{_ds // 60}:{_ds % 60:02d}")
                if pd.notna(_tl.get("pace_seconds")) and int(_tl["pace_seconds"]) > 0:
                    _pm, _ps = divmod(int(_tl["pace_seconds"]), 60)
                    _tl_parts.append(f"{_pm}:{_ps:02d}/mi")
                if pd.notna(_tl.get("elevation_gain_ft")) and float(_tl["elevation_gain_ft"]) > 0:
    ...
    if _has_logs and len(_rl_df) >= 2:
        st.divider()
        st.subheader("Snapshot")

        _an_df = _rl_df.sort_values("date").copy()

        _wk_start = today - timedelta(days=today.weekday())
        _wk_end = _wk_start + timedelta(days=6)
        _last_wk_start = _wk_start - timedelta(days=7)
        _last_wk_end = _wk_start - timedelta(days=1)

        _this_wk = _rl_df[(_rl_df["date"] >= _wk_start) & (_rl_df["date"] <= _wk_end)]
        _last_wk = _rl_df[(_rl_df["date"] >= _last_wk_start) & (_rl_df["date"] <= _last_wk_end)]
        _this_wk_mi = _this_wk["distance_miles"].sum() if not _this_wk.empty else 0
        _last_wk_mi = _last_wk["distance_miles"].sum() if not _last_wk.empty else 0
        _wk_delta = _this_wk_mi - _last_wk_mi
        ...
        _acwr = round(_ac / _ch, 2) if _ch > 0 else 0

        _pace_vals = _an_df["pace_seconds"].dropna()
        _last5_pace = _an_df.tail(5)["pace_seconds"].dropna().mean() if len(_an_df) >= 5 else _pace_vals.mean()
        _overall_pace = _pace_vals.mean() if not _pace_vals.empty else 0
        _pace_diff = _overall_pace - _last5_pace if _overall_pace > 0 and _last5_pace > 0 else 0

        _median_pace = _pace_vals.median() if not _pace_vals.empty else 0
        _easy_count = len(_pace_vals[_pace_vals > _median_pace * 1.05]) if _median_pace > 0 else 0
        _easy_pct = _easy_count / len(_pace_vals) * 100 if len(_pace_vals) > 0 else 0

        _sn1, _sn2, _sn3, _sn4 = st.columns(4)

        _sn1.metric(
            "This Week",
            f"{_this_wk_mi:.1f} mi",
            f"{_wk_delta:+.1f} vs last",
        )

        if _acwr > 1.5:
            _acwr_status = "High risk"
        elif _acwr > 1.3:
            _acwr_status = "Caution"
        elif _acwr >= 0.8:
            _acwr_status = "Sweet spot"
        else:
            _acwr_status = "Detraining"
        _sn2.metric("ACWR", f"{_acwr:.2f}", _acwr_status)

        _sn3.metric(
            "Avg Pace (Last 5)",
            _fmt_pace(_last5_pace) if _last5_pace > 0 else "--",
            f"{_pace_diff:+.0f}s vs overall" if _pace_diff != 0 else "same as overall",
        )

        _sn4.metric(
            "Easy Runs",
            f"{_easy_pct:.0f}%",
            "aim ~80%" if _easy_pct < 70 else "good balance",
        )

    st.divider()

    # ============================================================
    # FULL SCHEDULE + PROGRESS
    # ============================================================
...
if _has_logs and len(_rl_df) >= 2:
    ...
    _tab_mileage, _tab_pace, _tab_load, _tab_effort, _tab_splits, _tab_routes, _tab_prs = st.tabs(
        ["Mileage", "Pace", "Training Load", "Effort", "Splits", "Routes", "PRs"]
    )
    ...
    with _tab_pace:
        if len(_pace_df) >= 2:
            ...
            st.info("Need at least 2 runs with pace data.")
    ...
    with _tab_load:
        ...
        _load_chart = pd.DataFrame({
            "date": _load_dates,
            "ACWR": _acwr_vals,
        }).set_index("date")

        st.line_chart(_load_chart)
    ...
    with _tab_effort:
        if not _pace_df.empty:
            ...
            st.info("No pace data to analyze.")
    ...
    with _tab_splits:
        ...
        if not _split_runs.empty:
            ...
            st.info("No split data yet. Log runs with GPX for per-mile splits.")
    ...
    with _tab_routes:
        ...
        if not _named.empty:
            ...
            if not _repeat_routes.empty:
                ...
                st.info("Run the same route more than once to see comparisons.")
        else:
            st.info("Name your routes when logging to compare them over time.")
...
if _rl_data is None:
    ...
    _wk_start = today - timedelta(days=today.weekday())
    _wk_end = _wk_start + timedelta(days=6)
    _rl_week = _rl_df[(_rl_df["date"] >= _wk_start) & (_rl_df["date"] <= _wk_end)]
    ...
    if len(_rl_sorted) > 5:
        with st.expander(f"All Runs ({len(_rl_sorted)})"):
            _rest_runs = _rl_sorted.iloc[5:]
            _cur_year = None
            for _, _run in _rest_runs.iterrows():
                _yr = _run["date"].year
                if _yr != _cur_year:
                    _cur_year = _yr
